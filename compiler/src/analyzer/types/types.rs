...
impl Analyzer {
    ...
    fn resolve_array_type(
        ...
        let resolved = self.resolve_inner_type(module_id, ty)?;
        let mut size = None;
    ...
    pub(in crate::analyzer) fn resolve_fn_type(
        ...
        let args = fn_ty
            .args
            .iter()
            .map(|arg| self.resolve_inner_type(module_id, &arg.inner.inner))
            .collect::<Result<Vec<_>>>()?;
        let return_type = self.resolve_inner_type(module_id, &fn_ty.return_type.inner.inner)?;
    ...
    fn resolve_inner_type(
        ...
        let ty = match parser_type {
            ...
            TypeInner::Boxed(inner) => {
                let resolved = self.resolve_inner_type(module_id, &inner.inner)?;
                ResolvedType::Boxed(resolved)
            }

            TypeInner::FunctionType(f) => return self.resolve_fn_type(module_id, f),
    ...
    pub(in crate::analyzer) fn resolve_type(
        &mut self,
        ty: &Node<Type>,
        module_id: ModuleId,
    ) -> Result<TypeId> {
        let ty_inner = &ty.inner.inner;
        let resolved = self.resolve_inner_type(module_id, ty_inner)?;
